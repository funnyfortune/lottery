<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="configList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="参数键值" align="center" :show-overflow-tooltip="true" prop="configValue" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="50%" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="参数键名" prop="configKey">
          <el-input v-model="form.configKey" placeholder="请输入参数键名" :disabled="form.configId>0" />
        </el-form-item>
        ...
        <el-form-item label="参数键值" prop="configValue">
          <json-editor ref="jsonEditor" v-model="form.configValue"/>
        </el-form-item>
        <el-form-item label="系统内置" prop="configType">
...
export default {
  ...
  created() {
    this.getList();
    this.getDicts("sys_text_type").then(response => {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listConfig(this.addDateTimeRange(this.queryParams, this.dateRange)).then(response => {
    ...
    handleUpdate(row) {
      ...
      getConfig(configId).then(response => {
        if (response.data.valueType === '1') {
          response.data.configValue = JSON.parse(response.data.configValue)
        }
    ...
    /** 提交按钮 */
    submitForm: function() {
      this.form.configValue = this.$refs['jsonEditor'].getValue()
